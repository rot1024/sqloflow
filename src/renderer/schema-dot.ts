...
export const renderSchemaViewEnhanced = (graph: Graph): string => {
  const lines: string[] = [];
...
function renderSubquerySubgraph(subqueryNode: SubqueryNode, lines: string[], parentGraph: Graph): void {
  ...
  // Recursively render the inner graph using renderSchemaViewEnhanced
  const innerLines = renderSchemaViewEnhanced(subqueryNode.innerGraph).split('\n');
  ...
  // renderSchemaViewEnhanced call above, so we don't need to add them again
}

// Helper function to find the result node in a subquery graph
function findSubqueryResultNode(graph: Graph): Node | undefined {
  // Look for a node that represents the final result of the subquery
  // This could be a SELECT node, or the last node in the flow
  const selectNode = graph.nodes.find(n => n.kind === 'op' && n.label === 'SELECT');
  if (selectNode) return selectNode;
  
  // Otherwise, find the node with no outgoing edges
...
function getSubqueryResultLabel(subqueryType: string): string {
  switch (subqueryType) {
    case 'scalar':
      return 'scalar value';
    case 'in':
